...
// GPS 드리프터 통신 방식 — 소하천은 셀룰러로 충분(위성은 외해용·고비용).
export type DriftComm = "nbiot" | "ltem" | "gprs" | "satellite";
...
export interface SeacutObservation {
  ...
  track_id?: string | null; // 연계 GPS 드리프터 궤적(DriftTrack.track_id) 참조 — 표류 실측 라벨과 묶음
  // 시간
  ...
  photo_verified?: boolean | null; // 시민 관측 사진 검증 통과 여부(PlasticPirates식 4팀 검증) — 라벨 승격 게이트
  deid_flag: boolean; // 얼굴·번호판 비식별 처리 여부
  is_estimate: boolean; // 값이 추정치인지(자동 정확집계 미보장)
}

// GPS 드리프터 궤적 한 점. 좌표 시계열의 단위.
export interface DriftPoint {
  ts: string; // ISO8601 (+09:00)
  lat: number;
  lon: number;
  speed_est?: number | null; // 구간 표류 속도 추정(m/s)
}

// 드리프터 한 번의 투하~회수 궤적 = 표류 예측(litterRisk.ts)의 "어디로·어떻게" 정답 라벨.
// 수거 중량 앵커(collected_mass_kg)가 "얼마나"의 라벨이라면, 이쪽은 경로의 라벨이다.
// 제주 4개 하구 연구(동아대, 2025)의 발생원 역추적 지표(VSC·풍속계수 α)를 함께 담는다.
export interface DriftTrack {
  track_id: string; // 궤적 식별자(관측 레코드 SeacutObservation.track_id와 연결)
  drifter_id?: string | null; // 물리 드리프터 기기 ID(재사용 추적)
  site_id: string; // 연계 사이트(투하/회수 기준)
  comm?: DriftComm | null; // 통신 방식
  release_ts: string; // 투하 시각(ISO8601, +09:00)
  release_lat: number;
  release_lon: number;
  recovered_ts?: string | null; // 회수 시각(붐 종점 등) — 회수·재사용 원칙
  recovered_at_boom?: boolean | null; // 종점이 우리 붐인지(소하천=드리프터 종점)
  points: DriftPoint[]; // 좌표 시계열
  distance_m?: number | null; // 총 이동 거리(m)
  stranded?: boolean | null; // 제방·식생 고착 여부(갠지스 하천 고착률 40% 통찰)
  vsc_current?: number | null; // 벡터유사계수 — 해류 방향 일치도(제주 방법론)
  vsc_wind?: number | null; // 벡터유사계수 — 풍 방향 일치도(제주 방법론)
  wind_factor_alpha?: number | null; // 풍속계수 α = |드리프트속도-해류속도|/풍속(제주 방법론)
  is_estimate: boolean; // 표본 한계 — 단일 궤적은 전체 흐름의 추정 표본
}
...
const DRIFT_COMMS: DriftComm[] = ["nbiot", "ltem", "gprs", "satellite"];
...
export function validateObservation(rec: unknown): { ok: boolean; errors: string[] } {
  ...
  if (r.photo_verified != null && typeof r.photo_verified !== "boolean")
    e.push("photo_verified 는 boolean");
  return { ok: e.length === 0, errors: e };
}

// 드리프터 궤적 경량 검증 — 필수 식별·투하 정보와 좌표 시계열 형태만 확인.
export function validateDriftTrack(rec: unknown): { ok: boolean; errors: string[] } {
  const e: string[] = [];
  if (!rec || typeof rec !== "object") return { ok: false, errors: ["레코드가 객체가 아님"] };
  const r = rec as Record<string, unknown>;
  if (typeof r.track_id !== "string" || !r.track_id) e.push("track_id 필수(문자열)");
  if (typeof r.site_id !== "string" || !r.site_id) e.push("site_id 필수(문자열)");
  if (typeof r.release_ts !== "string" || !ISO_RE.test(r.release_ts as string))
    e.push("release_ts 필수(ISO8601)");
  if (typeof r.release_lat !== "number") e.push("release_lat 필수(number)");
  if (typeof r.release_lon !== "number") e.push("release_lon 필수(number)");
  if (!Array.isArray(r.points)) {
    e.push("points 필수(배열)");
  } else {
    r.points.forEach((p, i) => {
      if (!p || typeof p !== "object") {
        e.push(`points[${i}] 객체 아님`);
        return;
      }
      const pt = p as Record<string, unknown>;
      if (typeof pt.ts !== "string" || !ISO_RE.test(pt.ts as string))
        e.push(`points[${i}].ts ISO8601 아님`);
      if (typeof pt.lat !== "number") e.push(`points[${i}].lat number 아님`);
      if (typeof pt.lon !== "number") e.push(`points[${i}].lon number 아님`);
    });
  }
  if (r.comm != null && !DRIFT_COMMS.includes(r.comm as DriftComm)) e.push("comm 값 오류");
  if (typeof r.is_estimate !== "boolean") e.push("is_estimate 필수(boolean)");
  return { ok: e.length === 0, errors: e };
}

// JSONL 직렬화 — 한 줄 한 레코드(적재 표준 포맷). 관측·드리프터 궤적 공용.
export function toJsonl<T>(records: T[]): string {
  if (!records.length) return "";
  return records.map((r) => JSON.stringify(r)).join("\n") + "\n";
}

export { REGIMES, GRADES, ILLUMS, DRIFT_COMMS };
